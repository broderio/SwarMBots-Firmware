...
int read_header(spi_slave_transaction_t* t, uint8_t* header_data) {
    memcpy(&header_data[0], (uint8_t*)t->rx_buffer, ROS_HEADER_LEN);
    return 0;
}
...
int read_message(spi_slave_transaction_t* t, uint8_t* msg_data_serialized, uint16_t message_len, char* topic_msg_data_checksum) {
    memcpy(&msg_data_serialized[0], (uint8_t*)t->rx_buffer + ROS_HEADER_LEN, message_len);
    memcpy(topic_msg_data_checksum, (uint8_t*)t->rx_buffer + ROS_HEADER_LEN + message_len, 1);
    return 0;
}
